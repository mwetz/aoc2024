...
for i in 2800:length(memblocks)
    display(i)
    if isempty(follow_path(Trail([start], start, 0), simulate_corruption(tiles, memblocks, i)))
        blocking = memblocks[i] - CartesianIndex(1, 1)
        println(string(blocking[2]) * "," * string(blocking[1]))
        break
    end
end
